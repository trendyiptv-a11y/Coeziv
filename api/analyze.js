...
// ======================================================
// PRESENT REALITY GATE — UNIVERSAL CURRENT-TRUTH LAYER
// ======================================================
// Scop:
// Dacă adevărul afirmației depinde de prezent, starea curentă,
// titular actual, preț, versiune, lege în vigoare, rezultat,
// ghid actual, disponibilitate sau eveniment recent,
// memoria modelului NU este acceptată ca dovadă decisivă.
// ======================================================
...
function presentRealityGate(text = "") {
  ...
  const signals = [];
  const add = (x) => {
    if (!signals.includes(x)) signals.push(x);
  };

  const explicitPresent =
    /\b(actual|actuala|actuală|actualul|curent|curenta|curentă|prezent|prezenta|prezentă|azi|acum|recent|recente|ultim|ultima|ultimul|latest|current|incumbent|today|now|present|in force|în vigoare|in vigoare)\b/.test(t);

  const currentOfficeOrRole =
    /\b(președinte|presedinte|președintele|presedintele|președintelui|presedintelui|president|presidentul|prim[- ]?ministru|prim[- ]?ministrul|prim[- ]?ministrului|premier|premierul|premierului|ministru|ministrul|ministrului|guvernator|guvernatorul|governor|primar|primarul|mayor|ceo|ceo-ul|director|directorul|directorului|chairman|chairwoman|titular|titularul|titularului|lider|liderul|liderului|șef|sef|șeful|seful|șefului|sefului|conducător|conducator|conducătorul|conducatorul)\b/.test(t);

  const currentMarketValue =
    /\b(preț|pret|prețul|pretul|price|cotație|cotatie|cotația|cotatia|exchange rate|rata de schimb|curs valutar|dobândă|dobanda|dobânda|inflație|inflatie|inflația|inflatia|market cap|capitalizare|valoare actuală|valoare actuala|valoare curentă|valoare curenta|btc|bitcoin|eur|usd|dkk)\b/.test(t);

  const currentLawOrStandard =
    /\b(lege|legea|law|regulation|regulament|normă|norma|standard|ordin|ordonanță|ordonanta|directivă|directiva|directive|act normativ)\b/.test(t) &&
    /\b(vigoare|actual|actuală|actuala|curent|curentă|curenta|aplicabil|aplicabilă|aplicabila|se aplică|se aplica|valid|valabil|valabilă|valabila|in force|current)\b/.test(t);

  const currentTechnicalVersion =
    /\b(versiune|versiunea|version|release|build|api|sdk|library|bibliotecă|biblioteca|framework|standard tehnic|firmware|software)\b/.test(t) &&
    /\b(actual|actuală|actuala|curent|curentă|curenta|latest|current|nou|nouă|noua|ultima|ultimul|stable|release)\b/.test(t);

  const currentProductAvailability =
    /\b(disponibil|disponibilă|disponibila|disponibilitate|în stoc|in stoc|se mai vinde|available|availability|in stock|out of stock|sold out|preț produs|pret produs|ofertă|oferta|livrare|delivery)\b/.test(t);

  const currentSportsResult =
    /\b(scor|scorul|rezultat|rezultatul|clasament|clasamentul|standings|score|fixture|meci|meciul|a câștigat|a castigat|won|last match|ultimul meci|următorul meci|urmatorul meci)\b/.test(t);

  const currentMedicalGuideline =
    /\b(ghid actual|ghid curent|tratament actual|tratament curent|recomandat acum|contraindicat acum|current guideline|latest guideline|clinical guideline|standard of care|recomandare medicală actuală|recomandare medicala actuala)\b/.test(t);

  const currentEventStatus =
    /\b(alegeri|election|război|razboi|conflict|armistițiu|armistitiu|pace|acord|deal|negociere|negocieri|breaking|news|știre|stire|ultima oră|ultima ora|eveniment recent|criză|criza|a demisionat|demisie|numire|nominalizare|appointed|nominated|resigned)\b/.test(t);

  const currentWeatherOrTime =
    /\b(vreme|meteo|temperatura de azi|plouă|ploua|ninge|forecast|weather|ora actuală|ora actuala|time now|current time)\b/.test(t);

  if (explicitPresent) add("explicit_present");
  if (currentOfficeOrRole) add("current_office_or_role");
  if (currentMarketValue) add("current_market_value");
  if (currentLawOrStandard) add("current_law_or_standard");
  if (currentTechnicalVersion) add("current_version_or_standard");
  if (currentProductAvailability) add("current_product_availability");
  if (currentSportsResult) add("current_sports_result");
  if (currentMedicalGuideline) add("current_medical_guideline");
  if (currentEventStatus) add("current_event_status");
  if (currentWeatherOrTime) add("current_weather_or_time");

  const active = signals.length > 0;

  let relation_type = "stable_or_slow_relation";

  if (currentOfficeOrRole) relation_type = "current_office_holder";
  else if (currentMarketValue) relation_type = "current_market_value";
  else if (currentLawOrStandard) relation_type = "current_normative_status";
  else if (currentTechnicalVersion) relation_type = "current_version_or_standard";
  else if (currentProductAvailability) relation_type = "current_product_availability";
  else if (currentSportsResult) relation_type = "current_sports_result";
  else if (currentMedicalGuideline) relation_type = "current_medical_guideline";
  else if (currentEventStatus) relation_type = "current_event_status";
  else if (currentWeatherOrTime) relation_type = "current_weather_or_time";
  else if (explicitPresent) relation_type = "present_dependent_relation";

  const queries = [];

  if (active) {
    queries.push(
      `${raw} current official source`,
      `${raw} latest official`,
      `${raw} today current`,
      `${raw} official statement`,
      `${raw} Reuters AP current`
    );

    if (relation_type === "current_office_holder") {
      queries.push(
        `${raw} incumbent official`,
        `${raw} current office holder`,
        `${raw} government official current`,
        `${raw} official biography current`,
        `${raw} official government website`,
        `${raw} Reuters current`,
        `${raw} AP News current`
      );
    }

    if (relation_type === "current_market_value") {
      queries.push(
        `${raw} live price`,
        `${raw} current market data`,
        `${raw} official rate`,
        `${raw} current exchange rate`,
        `${raw} latest price`
      );
    }

    if (relation_type === "current_normative_status") {
      queries.push(
        `${raw} in force official`,
        `${raw} consolidated law current official`,
        `${raw} official gazette current`,
        `${raw} current regulation official`
      );
    }

    if (relation_type === "current_version_or_standard") {
      queries.push(
        `${raw} current version official docs`,
        `${raw} latest release official`,
        `${raw} official changelog`,
        `${raw} current standard official`
      );
    }

    if (relation_type === "current_product_availability") {
      queries.push(
        `${raw} availability official store`,
        `${raw} in stock current`,
        `${raw} current product page`,
        `${raw} official retailer availability`
      );
    }

    if (relation_type === "current_sports_result") {
      queries.push(
        `${raw} latest score official`,
        `${raw} current standings official`,
        `${raw} latest result official`
      );
    }

    if (relation_type === "current_medical_guideline") {
      queries.push(
        `${raw} current guideline WHO CDC EMA FDA`,
        `${raw} latest clinical guideline`,
        `${raw} current recommendation official`
      );
    }

    if (relation_type === "current_event_status") {
      queries.push(
        `${raw} latest Reuters`,
        `${raw} latest AP News`,
        `${raw} official statement latest`,
        `${raw} current status`
      );
    }

    if (relation_type === "current_weather_or_time") {
      queries.push(
        `${raw} current official forecast`,
        `${raw} weather today official`,
        `${raw} current time official`
      );
    }
  }
  ...
  return {
    active,
    stability: active ? "unstable_present" : "stable_or_slow",
    relation_type,
    signals,
    memory_allowed: !active,
    require_current_source: active,
    require_official_or_recent: active,
    require_date_check: active,
    require_relation_now: active,
    queries,
    reason: active
      ? "Afirmația depinde de prezent; memoria modelului este blocată ca dovadă decisivă. Relația trebuie verificată cu surse actuale/oficiale."
      : "Afirmația nu pare dependentă critic de prezent."
  };
}

function isPresentValidSource(source, text) {
  const pg = presentRealityGate(text);
  if (!pg.active) return true;

  const c = sourceText(source);
  const authority = Number(source?.authority_score || source?.authority_hint || 0);
  const dr = source?.domain_role || domainRole(source?.link);

  if (dr === "primary_or_specialized" && authority >= 5) return true;

  if (
    /\b(current|incumbent|today|latest|present|official|2025|2026|actual|curent|prezent|in office|sworn in|took office|appointed|nominated|resigned|live|latest release|in force)\b/.test(c) &&
    authority >= 4
  ) {
    return true;
  }
...
function presentGatePrompt(text, pg) {
  if (!pg?.active) {
    return "Ești Universal Coeziv Relational Resolver. Răspunde strict JSON valid. Nu verifica doar termenii; verifică relația completă subiect-relație-obiect.";
  }

  return [
    "Ești Present Reality Gate universal.",
    "Răspunde strict JSON valid.",
    "Afirmația depinde de prezent.",
    "Nu folosi memoria modelului și nu accepta date istorice ca realitate prezentă.",
    "Nu accepta titulari istorici, prețuri vechi, versiuni vechi, rezultate vechi, ghiduri vechi sau surse biografice drept adevăr prezent.",
    "Folosește numai surse actuale/oficiale din lista furnizată.",
    "Dacă sursele actuale arată un obiect real diferit de obiectul afirmat, two_pi_state='contradicted' și truth_consumption_mode='false_consumed'.",
    "Dacă sursele actuale confirmă relația, two_pi_state='confirmed' și truth_consumption_mode='true_consumed'.",
    "Dacă nu există sursă actuală suficientă, two_pi_state='incomplete' și truth_consumption_mode='incomplete'."
  ].join(" ");
}

// ======================================================
// INTEGRARE ÎN registryCategory(text) SAU truthType(text)
// Pune blocul acesta la începutul funcției registryCategory/truthType
// ======================================================

function presentTruthCategory(text) {
  const pg = presentRealityGate(text);

  if (!pg.active) return null;

  return {
    type: "present",
    label: "adevăr dependent de prezent",
    products: [
      "current official source",
      "latest reliable source",
      "official statement",
      "current relation",
      "present reality source"
    ],
    verifiers: [
      "sursă oficială actuală",
      "Reuters/AP/BBC recent",
      "date de piață actuale",
      "documentație oficială curentă",
      "registru oficial curent"
    ],
    max_if_primary: 5,
    present_gate: pg
  };
...
// În registryCategory(text) sau truthType(text), adaugă la început:
//
// const presentCategory = presentTruthCategory(text);
// if (presentCategory) return presentCategory;


// ======================================================
// INTEGRARE ÎN searchCandidates(text, atr)
// Adaugă blocul acesta după ce ai raw/text și lista qs
// ======================================================

function addPresentGateQueries(text, qs) {
  const pg = presentRealityGate(text);

  if (!pg.active) return qs;

  qs.unshift(...pg.queries);

  qs.push(
    `${text} current official`,
    `${text} latest official`,
    `${text} today current`,
    `${text} Reuters AP latest`,
...
// În searchCandidates(text, atr), folosește:
//
// qs = addPresentGateQueries(text, qs);


// ======================================================
// INTEGRARE ÎN classify(text, candidates, intent, atr)
// Pune blocul acesta înainte de a marca o sursă ca supporting_evidence
// ======================================================

function classifyPresentSource(source, text) {
  const pg = presentRealityGate(text);

  if (!pg.active) return null;

  const presentOK = isPresentValidSource(source, text);

  if (!presentOK) {
    return {
      role: "context_only",
      confidence: 0.15,
      reason: "sursă insuficient actuală pentru relație dependentă de prezent"
    };
  }
...
// În classify(...), înainte de regula supporting_evidence / official_validation:
//
// const presentOverride = classifyPresentSource(s, text);
// if (presentOverride) {
//   role = presentOverride.role;
//   confidence = presentOverride.confidence;
//   reason = presentOverride.reason;
// }


// ======================================================
// INTEGRARE ÎN relationClosure2Pi(text, sources, ctx, atr, F)
// Pune la începutul funcției
// ======================================================

function applyPresentGateToClosureBase(text, out) {
  const pg = presentRealityGate(text);

  if (!pg.active) return out;

  return {
    ...out,
    present_reality_gate: pg,
    relation_type: pg.relation_type
  };
}

// În relationClosure2Pi(...), după ce definești out:
//
// out = applyPresentGateToClosureBase(text, out);


// ======================================================
// FILTRARE SURSE ÎN relationClosure2Pi
// Înainte de AI fallback sau înainte de verificarea relației
// ======================================================

function presentUsableSources(text, allSources) {
  const pg = presentRealityGate(text);

  if (!pg.active) return allSources || [];

  return (allSources || []).filter((s) => isPresentValidSource(s, text));
}

// În relationClosure2Pi(...), folosește:
//
// const usableSources = presentUsableSources(text, all);


// ======================================================
// PROMPT AI PENTRU relationClosure2Pi
// Înlocuiește sys cu:
// ======================================================

function buildRelationClosureSystemPrompt(text) {
  const pg = presentRealityGate(text);
  return presentGatePrompt(text, pg);
}

// În relationClosure2Pi(...), folosește:
//
// const sys = buildRelationClosureSystemPrompt(text);


// ======================================================
// INTEGRARE ÎN consumption(...)
// Pune blocul acesta la începutul funcției consumption
// ======================================================

function presentConsumptionFallback(text, closure) {
  const pg = presentRealityGate(text);

  if (!pg.active) return null;

  const closed =
    closure?.two_pi_state === "confirmed" ||
    closure?.two_pi_state === "contradicted";
  ...
  return {
    level: 1,
    max: 5,
    label: "prezent neînchis complet",
    explanation:
      "Afirmația depinde de prezent. Memoria modelului nu este acceptată ca dovadă; este necesară o sursă actuală/oficială pentru închiderea relației.",
    primary_truth_product: {
      found: false,
      type: "present",
      label: "adevăr dependent de prezent",
      product: "present reality source",
      present_reality_gate: pg
    }
  };
}

// În consumption(...), pune la început:
//
// const presentFallback = presentConsumptionFallback(text, closure);
// if (presentFallback) return presentFallback;


// ======================================================
// INTEGRARE ÎN SUMMARY FINAL
// ======================================================

function applyPresentGateSummaryPrefix(text, summary) {
  const pg = presentRealityGate(text);

  if (!pg.active) return summary;

  return (
    "Afirmație dependentă de prezent: memoria modelului nu este folosită ca dovadă decisivă. " +
    summary
  );
}

// Înainte de return JSON:
//
// summary = applyPresentGateSummaryPrefix(text, summary);


// ======================================================
// INTEGRARE ÎN JSON FINAL
// În claim_context adaugă:
// ======================================================

function presentGateContext(text) {
  return presentRealityGate(text);
}
...
// claim_context: {
//   ...
//   present_reality_gate: presentGateContext(text),
//   ...
// }

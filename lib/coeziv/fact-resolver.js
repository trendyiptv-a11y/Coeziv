...
function compareObjects(claimed, resolved) {
  ...
  return { comparable: true, match: false, claimed_normalized: c, resolved_normalized: r };
}
...
function closureFromSemantic(claim, gate, semantic, sources) {
  const state = semantic.state || "incomplete";
  return {
    state,
...
export async function verifyClaim(text, env = process.env) {
  ...
  if (sources.length) {
    ...
    return { verifier: "semantic-ai", claim, gate, query, sources, semantic, resolved: { resolver: "semantic-ai", status: closure.state === "incomplete" ? "incomplete" : "resolved", resolved_object: closure.relation_object_found, explanation: closure.explanation }, closure, truth_consumption: buildTruthConsumption(closure) };
  }

  const resolved = { resolver: query ? "external-search" : "none", status: "incomplete", resolved_object: null, explanation: query ? "Nu au fost găsite surse externe pentru verificarea afirmației." : "Nu există resolver factual potrivit pentru această afirmație." };
